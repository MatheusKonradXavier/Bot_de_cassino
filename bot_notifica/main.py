...
def avisaBranco(lista):
  ...
  if(vezesBranco!=0 and vezesSemBranco!=0):
    ...
    if(vezesSemBranco in [50,60,70,80,90,100]) :
     resultado = botTelegram.enviarMensagem("🚨🚨🚨 HORA DE APOSTAR MERMÃO, " + str(vezesSemBranco) + " RODADAS SEM BRANCO 🚨🚨🚨")
  else :
    if(vezesBranco == 1):
      resultado = botTelegram.enviarMensagem("🌝🌝🌝 BRANQUINHO NA SUA CARA 🌝🌝🌝 ")
    else :
      ...
      try:
        ...
        geraLog.geraLog("ERRO AO SALVAR OS VALORES DO KITBLAZE AS " + datetime.now(timezone).strftime('%d/%m/%Y %H:%M:%S'))
  if(not resultado) :
    geraLog.geraLog("ERRO AO ENVIAR A MENSAGEM AS : " +  datetime.now(timezone).strftime('%H:%M'))
...
def start_notification():
  ...
  while(True) :
    try : 
      ...
      geraLog.geraLog("ERRO AO PEGAR OS VALORES DO KITBLAZE AS " + datetime.now(timezone).strftime('%d/%m/%Y %H:%M:%S'))
      atual = antigo.copy()
      time.sleep(1)
